structure(
  list(
    url = "https://vsim-dev.api.edp.aws.quartz.bio/v2/objects",
    status_code = 201L,
    headers = structure(
      list(
        date = "Mon, 19 Jan 2026 10:51:48 GMT",
        `content-type` = "application/json",
        `content-length` = "593",
        location = "None",
        server = "nginx",
        allow = "GET, POST, HEAD, OPTIONS",
        `x-frame-options` = "SAMEORIGIN",
        vary = "Authorization, Origin, Accept-Encoding",
        `content-encoding` = "gzip",
        `strict-transport-security` = "max-age=31536000; includeSubDomains",
        `solvebio-version` = "7.3.0-qb-int-dev"
      ),
      class = c("insensitive", "list")
    ),
    all_headers = list(list(
      status = 201L,
      version = "HTTP/2",
      headers = structure(
        list(
          date = "Mon, 19 Jan 2026 10:51:48 GMT",
          `content-type` = "application/json",
          `content-length` = "593",
          location = "None",
          server = "nginx",
          allow = "GET, POST, HEAD, OPTIONS",
          `x-frame-options` = "SAMEORIGIN",
          vary = "Authorization, Origin, Accept-Encoding",
          `content-encoding` = "gzip",
          `strict-transport-security` = "max-age=31536000; includeSubDomains",
          `solvebio-version` = "7.3.0-qb-int-dev"
        ),
        class = c("insensitive", "list")
      )
    )),
    cookies = structure(
      list(
        domain = logical(0),
        flag = logical(0),
        path = logical(0),
        secure = logical(0),
        expiration = structure(numeric(0), class = c("POSIXct", "POSIXt")),
        name = logical(0),
        value = logical(0)
      ),
      row.names = integer(0),
      class = "data.frame"
    ),
    content = charToRaw(
      "{\n  \"account_id\": 33,\n  \"ancestor_object_ids\": [],\n  \"availability\": {},\n  \"class_name\": \"Object\",\n  \"created_at\": \"2026-01-19T10:51:47.992737Z\",\n  \"dataset_description\": {},\n  \"dataset_documents_count\": {},\n  \"documents_count\": {},\n  \"dataset_full_name\": {},\n  \"storage_class\": {},\n  \"dataset_id\": {},\n  \"depth\": 0,\n  \"description\": \"\",\n  \"filename\": \"test-1\",\n  \"full_path\": \"vsim-test-dev:quartzbio.edp.test.Shortcut_get_target:/test-1\",\n  \"has_children\": false,\n  \"has_folder_children\": false,\n  \"id\": \"2754511072398390297\",\n  \"is_deleted\": false,\n  \"is_transformable\": false,\n  \"last_accessed\": \"2026-01-19T10:51:47.992687Z\",\n  \"last_modified\": \"2026-01-19T10:51:47.992687Z\",\n  \"md5\": {},\n  \"metadata\": {},\n  \"mimetype\": {},\n  \"num_children\": 0,\n  \"num_descendants\": 0,\n  \"object_type\": \"folder\",\n  \"parent_object_id\": {},\n  \"path\": \"/test-1\",\n  \"size\": {},\n  \"tags\": [],\n  \"updated_at\": \"2026-01-19T10:51:48.044702Z\",\n  \"upload_url\": {},\n  \"url\": {},\n  \"user\": {\n    \"class_name\": \"User\",\n    \"date_joined\": \"2023-01-27T16:17:17.087Z\",\n    \"email\": \"Test.User@fakemail.com\",\n    \"first_name\": \"Test\",\n    \"full_name\": \"Test User\",\n    \"id\": 51,\n    \"is_active\": true,\n    \"is_staff\": false,\n    \"is_verified\": true,\n    \"last_name\": \"User\",\n    \"password_days_remaining\": {},\n    \"role\": \"member\",\n    \"title\": \"\",\n    \"url\": \"https://vsim-dev.api.edp.aws.quartz.bio/v1/users/51\",\n    \"username\": \"mock_user\"\n  },\n  \"user_id\": 243,\n  \"vault_id\": 3178,\n  \"vault_name\": \"quartzbio.edp.test.Shortcut_get_target\",\n  \"version_count\": 0,\n  \"global_beacon\": {},\n  \"updated_by\": 243,\n  \"inherited_metadata\": {},\n  \"upload_id\": {},\n  \"upload_key\": {},\n  \"is_multipart\": false,\n  \"presigned_urls\": []\n}"
    ),
    date = structure(1768819908, class = c("POSIXct", "POSIXt"), tzone = "GMT"),
    times = c(
      redirect = 0,
      namelookup = 6.7e-05,
      connect = 0,
      pretransfer = 0.000247,
      starttransfer = 0.24813,
      total = 0.248397
    )
  ),
  class = "response"
)
