...
test_that_with_edp_api("file_download", {
  ...
  expect_identical(
    File_read(fi, local_path = local_path, overwrite = TRUE),
    "coucou"
  )
...
test_that_with_edp_api("File_upload works with mock API", {
  ...
  mockery::stub(
    File_upload,
    "request_edp_api",
    list(upload_url = "https://mock-upload-url")
  )
...
test_that_with_edp_api("Multi_part_file_upload works with mock API", {
  ...
  obj <- list(
    ...
    presigned_urls = list(
      list(
        ...
        size = 32 * 1024 * 1024,
        upload_url = "https://mock-url/part1"
      ),
      list(
        part_number = 2,
        start_byte = 32 * 1024 * 1024,
        end_byte = 65 * 1024 * 1024 - 1,
        size = 33 * 1024 * 1024,
        upload_url = "https://mock-url/part2"
      )
    ),
  ...
  mockery::stub(
    Multi_part_file_upload,
    "request_edp_api",
    list(message = "Multipart upload complete")
  )
